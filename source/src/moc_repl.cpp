/****************************************************************************
** Meta object code from reading C++ file 'repl.h'
**
** Created: Mon Sep 16 21:19:14 2013
**      by: The Qt Meta Object Compiler version 63 (Qt 4.8.4)
**
** WARNING! All changes made in this file will be lost!
*****************************************************************************/

#include "repl.h"
#if !defined(Q_MOC_OUTPUT_REVISION)
#error "The header file 'repl.h' doesn't include <QObject>."
#elif Q_MOC_OUTPUT_REVISION != 63
#error "This file was generated using the moc from 4.8.4. It"
#error "cannot be used with the include files from this version of Qt."
#error "(The moc has changed too much.)"
#endif

QT_BEGIN_MOC_NAMESPACE
static const uint qt_meta_data_REPL[] = {

 // content:
       6,       // revision
       0,       // classname
       0,    0, // classinfo
       4,   14, // methods
       0,    0, // properties
       0,    0, // enums/sets
       0,    0, // constructors
       0,       // flags
       0,       // signalCount

 // slots: signature, parameters, type, tag, flags
       5,   17,   17,   17, 0x08,
      18,   32,   17,   17, 0x08,

 // methods: signature, parameters, type, tag, flags
      37,   61,   65,   17, 0x02,
      73,   61,  105,   17, 0x02,

       0        // eod
};

static const char qt_meta_stringdata_REPL[] = {
    "REPL\0startLoop()\0\0stopLoop(int)\0code\0"
    "_getClassName(QObject*)\0obj\0QString\0"
    "_enumerateCompletions(QObject*)\0"
    "QStringList\0"
};

void REPL::qt_static_metacall(QObject *_o, QMetaObject::Call _c, int _id, void **_a)
{
    if (_c == QMetaObject::InvokeMetaMethod) {
        Q_ASSERT(staticMetaObject.cast(_o));
        REPL *_t = static_cast<REPL *>(_o);
        switch (_id) {
        case 0: _t->startLoop(); break;
        case 1: _t->stopLoop((*reinterpret_cast< const int(*)>(_a[1]))); break;
        case 2: { QString _r = _t->_getClassName((*reinterpret_cast< QObject*(*)>(_a[1])));
            if (_a[0]) *reinterpret_cast< QString*>(_a[0]) = _r; }  break;
        case 3: { QStringList _r = _t->_enumerateCompletions((*reinterpret_cast< QObject*(*)>(_a[1])));
            if (_a[0]) *reinterpret_cast< QStringList*>(_a[0]) = _r; }  break;
        default: ;
        }
    }
}

const QMetaObjectExtraData REPL::staticMetaObjectExtraData = {
    0,  qt_static_metacall 
};

const QMetaObject REPL::staticMetaObject = {
    { &QObject::staticMetaObject, qt_meta_stringdata_REPL,
      qt_meta_data_REPL, &staticMetaObjectExtraData }
};

#ifdef Q_NO_DATA_RELOCATION
const QMetaObject &REPL::getStaticMetaObject() { return staticMetaObject; }
#endif //Q_NO_DATA_RELOCATION

const QMetaObject *REPL::metaObject() const
{
    return QObject::d_ptr->metaObject ? QObject::d_ptr->metaObject : &staticMetaObject;
}

void *REPL::qt_metacast(const char *_clname)
{
    if (!_clname) return 0;
    if (!strcmp(_clname, qt_meta_stringdata_REPL))
        return static_cast<void*>(const_cast< REPL*>(this));
    return QObject::qt_metacast(_clname);
}

int REPL::qt_metacall(QMetaObject::Call _c, int _id, void **_a)
{
    _id = QObject::qt_metacall(_c, _id, _a);
    if (_id < 0)
        return _id;
    if (_c == QMetaObject::InvokeMetaMethod) {
        if (_id < 4)
            qt_static_metacall(this, _c, _id, _a);
        _id -= 4;
    }
    return _id;
}
QT_END_MOC_NAMESPACE
